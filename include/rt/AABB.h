...
struct AABB {
    ...
    bool hit(const Ray& r, double tmin, double tmax) const {
        for (int a = 0; a < 3; ++a) {
            ...
            double t0 = ((a == 0 ? min.x : (a == 1 ? min.y : min.z)) - (a==0? r.orig.x : (a==1? r.orig.y : r.orig.z))) * invD;
            double t1 = ((a == 0 ? max.x : (a == 1 ? max.y : max.z)) - (a==0? r.orig.x : (a==1? r.orig.y : r.orig.z))) * invD;
