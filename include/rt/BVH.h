...
struct BVHNode : public Hittable {
    ...
    BVHNode(std::vector<HittablePtr>& objects, size_t start, size_t end) {
        ...
        auto comparator = [axis](const HittablePtr& a, const HittablePtr& b){
            ...
            a->bounding_box(boxA);
            b->bounding_box(boxB);
            double ca = (axis==0? boxA.min.x + boxA.max.x : (axis==1? boxA.min.y + boxA.max.y : boxA.min.z + boxA.max.z));
            double cb = (axis==0? boxB.min.x + boxB.max.x : (axis==1? boxB.min.y + boxB.max.y : boxB.min.z + boxB.max.z));
            return ca < cb;
        };
...
private:
    static int choose_axis(std::vector<HittablePtr>& objs, size_t start, size_t end) {
        // pick axis with max variance of centers
        double mean[3] = {0,0,0};
        double m2[3] = {0,0,0};
        size_t n = 0;
        ...
        if (vary >= varx && vary >= varz) return 1;
        return 2;
    }
};
