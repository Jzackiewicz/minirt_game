...
inline bool parse_countable_token(const std::string &token, bool &countable)
{
        if (token.empty())
                return false;
        std::string upper;
        ...
        if (upper == "C" || upper == "COUNTABLE" || upper == "1" ||
                upper == "TRUE" || upper == "YES")
        {
                countable = true;
                return true;
        }
        if (upper == "NC" || upper == "NONCOUNTABLE" || upper == "0" ||
                upper == "FALSE" || upper == "NO")
        {
                countable = false;
                return true;
        }
                                               ...
                                               std::vector<Material> &mats)
{
        ...
        if (iss >> s_count)
                parse_countable_token(s_count, countable_flag);
        Vec3 c, rgb;
        double r = 1.0;
        double a = 255;
