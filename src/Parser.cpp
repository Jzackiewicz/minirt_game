...
inline bool is_countable_token(std::string token)
{
        for (char &ch : token)
                                               ...
                                               std::vector<Material> &mats)
{
        ...
        if (parse_triple(s_pos, c) && to_double(s_r, r) && parse_rgba(s_rgb, rgb, a))
        {
                ...
                s->movable = (s_move == "M");
                s->countable = is_countable_token(s_count);
                mats.emplace_back();
                                              ...
                                              std::vector<Material> &mats)
{
        ...
        if (parse_triple(s_p, p) && parse_triple(s_n, n) && parse_rgba(s_rgb, rgb, a))
        {
                ...
                pl->movable = (s_move == "M");
                pl->countable = is_countable_token(s_count);
                mats.emplace_back();
                mats.back().color = rgb_to_unit(rgb);
                mats.back().base_color = mats.back().color;
                mats.back().alpha = alpha_to_unit(a);
                                                   ...
                                                   std::vector<Material> &mats)
{
                ...
                to_double(s_d, d) && to_double(s_h, h) && parse_rgba(s_rgb, rgb, a))
        {
                ...
                cy->movable = (s_move == "M");
                cy->countable = is_countable_token(s_count);
                mats.emplace_back();
                mats.back().color = rgb_to_unit(rgb);
                mats.back().base_color = mats.back().color;
                mats.back().alpha = alpha_to_unit(a);
                                            ...
                                            std::vector<Material> &mats)
{
                ...
                parse_rgba(s_rgb, rgb, alpha))
        {
                ...
                cu->movable = (s_move == "M");
                cu->countable = is_countable_token(s_count);
                mats.emplace_back();
                mats.back().color = rgb_to_unit(rgb);
                mats.back().base_color = mats.back().color;
               ...
               to_double(s_r, ray_radius) && to_double(s_L, L))
        {
               ...
               auto bm = std::make_shared<Beam>(o, dir_norm, ray_radius, L,
                ...
                if (with_laser)
                {
                        ...
                        bm->source->countable = is_countable_token(s_count);
                                                          ...
                                                          int &mid, std::vector<Material> &mats)
{
        ...
        if (parse_triple(s_pos, c) && parse_rgba(s_rgb, rgb, a) && to_double(s_r, R))
        {
                ...
                bt->movable = (s_move == "M");
                bt->countable = is_countable_token(s_count);
                                           ...
                                           std::vector<Material> &mats)
{
                ...
                to_double(s_d, d) && to_double(s_h, h) && parse_rgba(s_rgb, rgb, a))
        {
                ...
                co->movable = (s_move == "M");
                co->countable = is_countable_token(s_count);
                mats.emplace_back();
                mats.back().color = rgb_to_unit(rgb);
                mats.back().base_color = mats.back().color;
                mats.back().alpha = alpha_to_unit(a);
...
// Serialize scene and camera to .rt file.
bool Parser::save_rt_file(const std::string &path, const Scene &scene,
						  const Camera &cam, const std::vector<Material> &mats)
{
	...
	for (const auto &obj : scene.objects)
	{
		if (obj->is_beam())
		{
			...
			if (auto src = bm->source.lock(); src && src->movable)
				move = "M";
                        std::string countable = "NC";
                        if (auto src = bm->source.lock())
                                countable = src->countable ? "C" : "NC";
	...
	for (const auto &obj : scene.objects)
	{
		...
		std::string mirror = m.mirror ? "R" : "NR";
                        ...
                        std::string countable = obj->countable ? "C" : "NC";
