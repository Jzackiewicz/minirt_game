...
namespace
{
	...
	void draw_character(SDL_Renderer *renderer, char character, int x, int y, SDL_Color color, int scale)
	{
		const uint8_t *glyph;
	...
	void draw_text(SDL_Renderer *renderer, const std::string &text, int x, int y, SDL_Color color, int scale)
	{
...
bool MainMenu::show(int width, int height)
{
	...
	SDL_Window *window;
	window = SDL_CreateWindow("MiniRT", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, 0);
	if (!window)
	{
		SDL_Quit();
		return false;
	...
	while (running)
	{
		...
		bool hover_play;
		hover_play = mouse_x >= play_rect.x && mouse_x < play_rect.x + play_rect.w && mouse_y >= play_rect.y && mouse_y < play_rect.y + play_rect.h;
		bool hover_settings;
		hover_settings = mouse_x >= settings_rect.x && mouse_x < settings_rect.x + settings_rect.w && mouse_y >= settings_rect.y && mouse_y < settings_rect.y + settings_rect.h;
		SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
		SDL_RenderClear(renderer);
		SDL_Color fill;
		fill = hover_play ? SDL_Color{0, 128, 128, 255} : SDL_Color{0, 0, 0, 255};
		...
		fill = hover_settings ? SDL_Color{0, 128, 128, 255} : SDL_Color{0, 0, 0, 255};
		SDL_SetRenderDrawColor(renderer, fill.r, fill.g, fill.b, fill.a);
