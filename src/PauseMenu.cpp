...
bool PauseMenu::show(SDL_Window *window, SDL_Renderer *renderer, int width, int height)
{
    ...
    if (capture)
    {
        ...
        {
            background = SDL_CreateTextureFromSurface(renderer, capture);
        }
    ...
    SDL_Rect leaderboard_rect = {center_x, resume_rect.y + button_height + button_gap,
                                 button_width, button_height};
    SDL_Rect settings_rect = {center_x, leaderboard_rect.y + button_height + button_gap,
                              button_width, button_height};
    ...
    while (running)
    {
        ...
        for (std::size_t i = 0; i < title.size(); ++i)
        {
            ...
            if (i == 0)
                ...
                color = {0, 255, 255, 255};
            draw_character(renderer, title[i], x, title_y, color, title_scale);
            x += (5 + 1) * title_scale;
        }
