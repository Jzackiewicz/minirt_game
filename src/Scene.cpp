...
{
        constexpr double cam_radius = 0.2;

        auto collides = [&](const HittablePtr &sph) {
                for (const auto &obj : objects)
                {
                        ...
                        if (mat.alpha < 1.0)
                                continue;
                        if (precise_collision(sph, obj))
                                return true;
                }
                return false;
        };

        auto sphere = std::make_shared<Sphere>(cam.origin, cam_radius, -1, 0);
