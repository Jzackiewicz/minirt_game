...
namespace {
// 8x8 font map for ASCII characters
...
const uint8_t *CustomCharacter::get_glyph(unsigned char character) {
    return font8x8_basic[character];
}

void CustomCharacter::draw_character(SDL_Renderer *renderer, char character, int x, int y,
                                     SDL_Color color, int scale) {
    const uint8_t *glyph = get_glyph(static_cast<unsigned char>(character));
...
int CustomCharacter::text_width(const std::string &text, int scale) {
    return (static_cast<int>(text.size()) * (8 + 1) - 1) * scale;
}
