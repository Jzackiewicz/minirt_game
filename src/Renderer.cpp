...
int Renderer::render_hud(const RenderState &st, SDL_Renderer *ren, int W, int H)
{
        ...
        constexpr size_t kControlSectionCount = 5;
        std::array<HudTextLine, kControlSectionCount> control_slots{};
        std::array<bool, kControlSectionCount> slot_used{};
        auto set_control = [&](size_t index, const std::string &label, SDL_Color color) {
                if (index >= control_slots.size())
                        return;
                control_slots[index] = {label, color};
                slot_used[index] = !label.empty();
        };
        ...
        set_control(kControlSectionCount - 1, "Pause: ESC", neutral);

        if (!st.focused)
        {
                set_control(0, "Focus lost", warning);
                set_control(1, "Click to focus", neutral);
        }
        else if (st.edit_mode)
        {
                ...
                set_control(0, "Move camera", neutral);
                if (can_rotate)
                {
                        set_control(1,
                ...
                set_control(2, "Place: LPM", accent);

                std::string object_slot_label;
                if (can_move)
                        object_slot_label = "Move object: mouse + scroll";
                if (g_developer_mode && selected_obj)
                {
                        if (!object_slot_label.empty())
                                object_slot_label += "; Resize: scroll";
                        else
                                object_slot_label = "Resize: scroll";
                }
                if (!object_slot_label.empty())
                        set_control(3, object_slot_label, neutral);

                if (g_developer_mode && selected_obj)
                {
                        if (!slot_used[1])
                                set_control(1, "Delete: MMB", danger);
                        else if (!slot_used[3])
                                set_control(3, "Delete: MMB", danger);
                        else if (!slot_used[2])
                                set_control(2, "Delete: MMB", danger);
                }
        }
        else
        {
                set_control(0, "Move camera: WSAD", neutral);
                set_control(1, "Ascend: SPACE", neutral);
                set_control(2, "Descend: CTRL", neutral);
        ...
        double section_span = static_cast<double>(W) / kControlSectionCount;
        SDL_SetRenderDrawColor(ren, 255, 255, 255, 96);
        for (size_t i = 0; i < kControlSectionCount; ++i)
        {
                ...
                {
                        int width =
                                CustomCharacter::text_width(control_slots[i].text, hud_scale);
                        ...
                        CustomCharacter::draw_text(ren, control_slots[i].text, text_x, controls_y,
                                                    control_slots[i].color, hud_scale);
                }
                if (i + 1 < kControlSectionCount)
