...
int Renderer::render_hud(const RenderState &st, SDL_Renderer *ren, int W, int H)
{
        ...
        for (const auto &section : control_sections)
        {
                if (section && !section->text.empty())
                        max_control_lines =
                                std::max(max_control_lines, count_lines(section->text));
        }
        ...
        for (size_t i = 0; i < control_sections.size(); ++i)
        {
                ...
                {
                        lines = split_lines(control_sections[i]->text);
                        for (const auto &line : lines)
                        {
                                int line_width = line.empty()
                                                         ? 0
                                                         : CustomCharacter::text_width(line,
                                                                                         hud_scale);
                                block_width = std::max(block_width, line_width);
                        }
                }
                int centered = start_x + (available - block_width) / 2;
                int min_x = start_x + hud_padding;
                int max_x = end_x - hud_padding - block_width;
                int block_x = std::clamp(centered, min_x, std::max(min_x, max_x));
                if (!lines.empty())
                {
                        ...
                        for (const auto &line : lines)
                        {
                                int line_width = line.empty()
                                                         ...
                                                         : CustomCharacter::text_width(line,
                                                                                         hud_scale);
                                int line_x = block_x + (block_width - line_width) / 2;
                                if (!line.empty())
