...
Vec3 surface_color_at_hit(const Scene &scene, const std::vector<Material> &mats,
                                                 const HitRecord &rec)
{
                                                         ...
                                                         double sample_weight)
{
        ...
        while (travelled < max_range - 1e-4 && transmittance > 1e-4)
        {
                ...
                const Material &mat = mats[rec.material_id];
                Vec3 surface_color = surface_color_at_hit(scene, mats, rec);
                ...
                if (hit_obj && hit_obj->scorable && !hit_obj->is_beam())
                {
                        ...
                        if (dist2 > 1e-8)
                        {
                                ...
                                if (cos_incident > 1e-6)
                                {
                                        ...
                                        if (L.range > 0.0)
                                        {
                                                double falloff = 1.0 - dist / L.range;
                                                if (falloff <= 0.0)
                                                        attenuation = 0.0;
                                                else
                                                        attenuation = falloff;
                                        }
                                        double incident_intensity =
                                                L.intensity * transmittance;
                                        Vec3 tinted_light = beam_color;
                                        double diff_term = incident_intensity *
                                                                   cos_incident * attenuation;
                                        Vec3 diffuse(diff_term * surface_color.x *
                                                                 tinted_light.x,
                                                     diff_term * surface_color.y *
                                                                 tinted_light.y,
                                                     diff_term * surface_color.z *
                                                                 tinted_light.z);
                                        ...
                                        Vec3 specular = tinted_light *
                                                        (spec_factor * attenuation *
                                                         incident_intensity);
                                        Vec3 brightness = diffuse + specular;
                                        double luminance =
                                                0.2126 * brightness.x + 0.7152 * brightness.y +
                                                0.0722 * brightness.z;
                                        if (luminance > 1e-9 && area > 0.0)
                                                total_brightness += area * luminance;
                                }
                        }
                }
                ...
                beam_color = mix_colors(beam_color, surface_color, mat.alpha);
