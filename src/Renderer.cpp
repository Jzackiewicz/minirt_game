...
int Renderer::render_hud(const RenderState &st, SDL_Renderer *ren, int W, int H)
{
        ...
        std::array<HudTextLine, 5> control_sections;
        for (auto &section : control_sections)
                section = HudTextLine{"", neutral};

        auto set_section = [&](size_t index, const std::string &label, SDL_Color color) {
                if (index < control_sections.size())
                        control_sections[index] = {label, color};
        };

        set_section(4, "Pause: ESC", neutral);

        if (!st.focused)
        {
                set_section(0, "FOCUS LOST", warning);
                set_section(1, "CLICK TO FOCUS", neutral);
        }
        else if (st.edit_mode)
        {
                set_section(0, "Move camera", neutral);
                ...
                if (selected_obj)
                {
                        bool can_move = g_developer_mode ||
                        ...
                        set_section(2, "Place: LPM", accent);

                        std::string object_section;
                        if (can_move)
                                object_section = "Move object: mouse + scroll";

                        if (g_developer_mode)
                        {
                                if (!object_section.empty())
                                        object_section += "; ";
                                object_section += "Resize: scroll";
                                object_section += "; Delete: MMB";
                        }

                        if (!object_section.empty())
                        {
                                SDL_Color object_color = can_move ? accent : neutral;
                                set_section(3, object_section, object_color);
                        }
                }
        }
        else
        {
                set_section(0, "Move: WSAD", neutral);
                set_section(1, "Ascend: SPACE", neutral);
                set_section(2, "Descend: CTRL", neutral);
