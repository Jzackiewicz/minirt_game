...
int Renderer::render_hud(const RenderState &st, SDL_Renderer *ren, int W, int H)
{
        ...
        {
                SDL_SetRenderDrawColor(ren, 180, 180, 180, 128);
        ...
        for (size_t pos = 0; pos < active_sections.size(); ++pos)
        {
                ...
                {
                        int separator_top = controls_top + hud_padding / 2;
                        int separator_bottom = H - hud_padding / 2;
                        if (separator_bottom <= separator_top)
                        {
                                separator_top = H - bottom_bar_height;
                                separator_bottom = H;
                        }
                        SDL_RenderDrawLine(ren, end_x, separator_top, end_x,
                                           separator_bottom);
                }
        }
