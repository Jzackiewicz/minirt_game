...
int Renderer::render_hud(const RenderState &st, SDL_Renderer *ren, int W, int H)
{
        ...
        SDL_SetRenderDrawColor(ren, 32, 32, 32, 200);
        ...
        int control_bar_margin = std::max(2, hud_padding / 2);
        int control_rect_top = H - bottom_bar_height + control_bar_margin;
        int control_rect_bottom = H - control_bar_margin;
        int control_rect_height = std::max(0, control_rect_bottom - control_rect_top);
        int controls_top = control_rect_top;
        if (control_rect_height > 0)
        {
                int total_text_height = static_cast<int>(max_control_lines) * hud_line_height;
                int extra_space = control_rect_height - total_text_height;
                if (extra_space > 0)
                        controls_top += extra_space / 2;
        }
        ...
        for (size_t pos = 0; pos < active_sections.size(); ++pos)
        {
                ...
                {
                        int rect_x = start_x + control_bar_margin;
                        int rect_w = available - 2 * control_bar_margin;
                        if (rect_w < 0)
                        {
                                rect_x += rect_w / 2;
                                rect_w = 0;
                        }
                        SDL_Rect section_rect{rect_x, control_rect_top, rect_w,
                                              control_rect_height};
                        SDL_SetRenderDrawColor(ren, 64, 64, 64, 200);
                        SDL_RenderFillRect(ren, &section_rect);
