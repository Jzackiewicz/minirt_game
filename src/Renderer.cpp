                                                       ...
                                                       std::vector<Material> &mats)
{
        ...
        char score_buf[64];
        std::snprintf(score_buf, sizeof(score_buf), "SCORE: %.1f", display_score);
        CustomCharacter::draw_text(ren, score_buf, 5, 5, score_color, score_scale);
        int developer_start = 5 + score_scale * 7 + 4;
        ...
        if (g_developer_mode)
        {
                ...
                for (int i = 0; i < 7; ++i)
                        CustomCharacter::draw_text(ren, legend[i], 5,
                                                    developer_start + i * (7 * scale + 2), red,
                                                    scale);
