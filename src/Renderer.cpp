...
int Renderer::render_hud(const RenderState &st, SDL_Renderer *ren, int W, int H)
{
        ...
        struct HudControlSection
        {
                std::string text;
                SDL_Color color{neutral.r, neutral.g, neutral.b, neutral.a};
                bool active{false};
        };

        constexpr size_t kHudControlSlotCount = 5;
        std::array<HudControlSection, kHudControlSlotCount> control_slots;
        auto set_slot = [&](size_t idx, const std::string &label, SDL_Color color) {
                if (idx >= control_slots.size())
                        return;
                control_slots[idx].text = label;
                control_slots[idx].color = color;
                control_slots[idx].active = !label.empty();
        };
        if (!st.focused)
        {
                set_slot(0, "Click to focus", warning);
        }
        else if (st.edit_mode)
        {
                ...
                set_slot(0, "Move camera: WSAD/SPACE/CTRL", neutral);
                ...
                set_slot(2, "Place: LPM", accent);

                std::string manipulation_line;
                if (can_move)
                        manipulation_line = "Move object: mouse + scroll";
                else if (!g_developer_mode && selected_obj)
                        manipulation_line = "Adjust distance: scroll";

                if (g_developer_mode && selected_obj)
                {
                        if (!manipulation_line.empty())
                                manipulation_line += "; ";
                        manipulation_line += "Resize: scroll";

                        if (!manipulation_line.empty())
                                manipulation_line += "; ";
                        manipulation_line += "Delete: MMB";
        }

                if (!manipulation_line.empty())
                        set_slot(3, manipulation_line, neutral);
        }
        else
        {
                set_slot(0, "Move: WSAD/SPACE/CTRL", neutral);
                ...
                if (show_grab)
                        set_slot(2, "Grab: LPM", accent);
        }

        set_slot(4, "Pause: ESC", neutral);
        ...
        size_t section_count = control_slots.size();
        ...
        for (size_t i = 0; i < section_count; ++i)
        {
                ...
                if (control_slots[i].active)
                {
                        ...
                        CustomCharacter::draw_text(ren, control_slots[i].text, text_x,
                                                    controls_y, control_slots[i].color,
                                                    hud_scale);
                }
